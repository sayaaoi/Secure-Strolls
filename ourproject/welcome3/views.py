    ...
    # req = requests.get(create_request(start_location, end_location))
            ...
            for route in routes:
                ...
                for step in steps:
                    ...
                    cursor.execute(
                        'SELECT SUM(Score) FROM INCIDENT JOIN Crime ON  INCIDENT.Crime_Type = Crime.Crime_Type JOIN LOCATION ON INCIDENT.Location_ID = LOCATION.Location_ID WHERE Latitude > {0} and Latitude < {1} and Longitude > {2} and Longitude < {3};'.format(
                            s_lat, e_lat, s_long, e_long))
                    print(type(cursor.fetchall()))
                    stepScore += list(cursor.fetchall()[0])
                    ...
                    #     checkarr.append(row)
                i += 1
                scoreDict['Route'+i] = [stepScore, stepAddress]
            # Find the safest route based on score
            sortedRoute = sorted(scoreDict.items(), key=lambda k:k[0])
            bestRoute = sortedRoute[0][1][1]
            
            # arr = [check, check2, checkarr]
            return TemplateResponse(request, 'welcome/result.html', {"test": bestRoute})
    else:
        loc_form = LocationForm()
    return TemplateResponse(request, 'welcome/enterloc.html', {"test": bestRoute})
